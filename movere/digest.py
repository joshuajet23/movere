...
def _build_coaching_prompt(fitness: dict, calendar_data: dict) -> str:
...
def generate_coaching_note(cfg: dict, fitness: dict, calendar_data: dict) -> str:
    client = anthropic.Anthropic(api_key=cfg["anthropic"]["api_key"])
    prompt = _build_coaching_prompt(fitness, calendar_data)
...
def render_html(date_str: str, coaching_note: str, fitness: dict, calendar_data: dict, lookahead_days: int) -> str:
