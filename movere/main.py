...
from . import config, digest, email_sender, garmin, screentime, chat, server as web_server, sync
...
def main() -> None:
    ...
    # movere sync
    sub.add_parser("sync", help="Push Mac-only data (screen time) to GitHub before cloud digest runs")
    ...
    if args.cmd == "project":
        ...
        sync.push_screentime(config.load())
